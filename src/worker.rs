...
pub async fn run(agent_name: String, config_path: String) {
    ...
    let agent_config = match config.agents.iter().find(|a| a.name == agent_name) {
        ...
        None => {
            send_status(
                "error",
                Some(&format!("agent '{}' not found in config", agent_name)),
            );
            std::process::exit(1);
        }
    };
        ...
        match cmd.get("cmd").and_then(|v| v.as_str()) {
            ...
            Some("job_execute") => {
                ...
                let workspace = cmd
                    ...
                    .unwrap_or("/tmp")
                    .to_string();
                let input_artifacts: Vec<String> = cmd
                    .get("input_artifacts")
                ...
                send_progress(
                    &job_id,
                    "preparing",
                    "Building context from input artifacts",
                    None,
                );
            ...
            Some("stop") => {
                ...
                if let Some(ref path) = session_file {
                    ...
                    send_log(
                        "info",
                        &format!("session saved ({} messages)", history.len()),
                    );
                }
...
fn send_progress(job_id: &str, phase: &str, message: &str, percent: Option<u8>) {
    let mut msg =
        serde_json::json!({"type":"progress","job_id":job_id,"phase":phase,"message":message});
